...
# Create blueprint
viz_bp = Blueprint('visualization', __name__, url_prefix='/visualization')
...
def create():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if viz_type == 'bar':
                config['additional_options']['orientation'] = request.form.get('bar_orientation', 'v')
            elif viz_type == 'scatter':
                config['additional_options']['marker_size'] = request.form.get('marker_size', 6, type=int)
                config['additional_options']['trendline'] = 'trendline' in request.form
            elif viz_type == 'line':
                config['additional_options']['line_shape'] = request.form.get('line_shape', 'linear')
            elif viz_type == 'pie':
                config['additional_options']['hole'] = request.form.get('pie_hole', 0, type=float)
            ...
            return redirect(url_for('visualization.view', viz_id=viz_id))
...
def delete(viz_id):
    ...
    success = delete_visualization(viz_id)
    if success:
        flash('Visualization deleted successfully', 'success')
    else:
        flash('Error deleting visualization', 'error')
    return redirect(url_for('visualization.index'))
...
def generate_visualization(data, config):
    ...
    if viz_type == 'bar':
        ...
        fig = px.line(df, x=x_column, y=y_column, color=color_column, title=title, line_shape=line_shape)
        
    elif viz_type == 'scatter':
        marker_size = additional_options.get('marker_size', 6)
        trendline = additional_options.get('trendline', False)
        fig = px.scatter(df, x=x_column, y=y_column, color=color_column, title=title, trendline='ols' if trendline else None)
...
def create_visualization(name, description, viz_type, result_id, config):
    ...
    Returns:
        viz_id: ID of the created visualization
    """
...
# Include only basic CSS and template generation
def generate_css():
    """Generate minimal CSS for the visualization module"""
    css = """
    ...
    with open('static/css/visualization_module.css', 'w') as f:
        f.write(css)

def generate_js():
    """Generate minimal JavaScript for the visualization module"""
    js = """
    ...
    with open('static/js/visualization_module.js', 'w') as f:
        f.write(js)

def generate_templates():
    """Generate basic templates for the visualization module"""
    # Create the directory if it doesn't exist
    os.makedirs('templates', exist_ok=True)
    
    # Create a minimal template for the visualization index
    index_template = """
    {% extends 'base.html' %}
    ...
    <div class="container mt-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <a href="{{ url_for('visualization.create') }}" class="btn btn-primary">
                ...
                <div class="col-md-4 mb-4">
                    <div class="card viz-card h-100">
                        ...
                        <div class="card-body">
                            ...
                            <p class="card-text"><small class="text-muted">Created {{ viz.created_at }}</small></p>
                        </div>
                        <div class="card-footer bg-transparent">
                            <a href="{{ url_for('visualization.view', viz_id=viz.id) }}" class="btn btn-primary btn-sm">View</a>
                            <form method="POST" action="{{ url_for('visualization.delete', viz_id=viz.id) }}" class="d-inline">
                                <button type="submit" class="btn btn-danger btn-sm delete-viz-btn">Delete</button>
                            </form>
                        </div>
                    </div>
                </div>
    ...
    # Create a minimal template for the create visualization page
    create_template = """
    {% extends 'base.html' %}
        ...
        <div class="card">
            <div class="card-body">
                <form method="POST">
                    ...
                    <!-- Bar Chart Options -->
                    <div id="bar-options" class="mb-3 viz-options-section">
                        <label class="form-label">Bar Chart Options</label>
                    ...
                    <!-- Line Chart Options -->
                    <div id="line-options" class="mb-3 viz-options-section" style="display:none;">
                        <label class="form-label">Line Shape</label>
                        <select class="form-control" name="line_shape">
                    ...
                    <div id="scatter-options" class="mb-3 viz-options-section" style="display:none;">
                        <label class="form-label">Scatter Plot Options</label>
                    ...
                    <!-- Pie Chart Options -->
                    <div id="pie-options" class="mb-3 viz-options-section" style="display:none;">
                        <label class="form-label">Pie Chart Options</label>
                        <div class="form-group">
                            <label for="pie-hole">Donut Hole Size (0-1)</label>
                    ...
                    <div class="mb-3">
                        ...
                        <a href="{{ url_for('visualization.index') }}" class="btn btn-outline-secondary">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
    ...
    {% block scripts %}
    <script src="{{ url_for('static', filename='js/visualization_module.js') }}"></script>
    {% endblock %}
    """
    
    # Create a minimal template for viewing a visualization
    view_template = """
    {% extends 'base.html' %}
                ...
                <div class="row">
                    <div class="col-md-6">
                        <dl class="row mb-0">
                            ...
                            <dd class="col-sm-8">{{ visualization.type|title }} Chart</dd>
                        </dl>
                    </div>
                    <div class="col-md-6">
                        <dl class="row mb-0">
                            ...
                            <dd class="col-sm-8">{{ visualization.created_at }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
    ...
    # Simple data selection template
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ url_for('visualization.create', result_id=result.id) }}" class="btn btn-primary btn-sm">
